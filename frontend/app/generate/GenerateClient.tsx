...
export default function GeneratePageClient({ availableVersions }: GeneratePageClientProps) {
  ...
  const generateEdifact = (version: string, targetBytes: number): string => {
    ...
    // Generate items until we reach target size
    let currentBytes = 0;
    const headerLines = lines.slice();
    let headerText = headerLines.join('\n');
    // Add newline after each line
    currentBytes = new TextEncoder().encode(headerText + '\n').length;
    
    const itemLines = [];
    let i = 1;
    while (currentBytes < targetBytes) {
      // LIN: Line item
      itemLines.push(segment('LIN', i.toString(), '', '', `SKU${i.toString().padStart(6, '0')}`));
      
      // QTY: Quantity
      const qty = Math.floor(Math.random() * 1000) + 1;
      itemLines.push(segment('QTY', '1', `${qty}`, 'PCE'));
      
      // PRI: Price
      const price = (Math.random() * 100).toFixed(2);
      itemLines.push(segment('PRI', 'AAA', price));
      
      // Calculate size of this item block (3 lines + newlines)
      const blockText = itemLines.slice(-3).join('\n') + '\n';
      currentBytes += new TextEncoder().encode(blockText).length;
      i++;
    }
    
    // If we overshot, remove last item
    if (currentBytes > targetBytes) {
      itemLines.splice(-3, 3);
      // Recalculate currentBytes
      const allLines = headerLines.concat(itemLines);
      const fullText = allLines.join('\n') + '\n';
      currentBytes = new TextEncoder().encode(fullText).length;
    }
    ...
    return finalText + (finalText.endsWith('\n') ? '' : '\n');
  };
